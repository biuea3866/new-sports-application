/**
 * [U-01] ~ [U-08] parseCsvFacilities 단위 테스트
 */
import { describe, it, expect } from "vitest";
import { parseCsvFacilities } from "../parseCsvFacilities";

const VALID_HEADER = "code,name,gu,type,address,lat,lng,parking,tel,homePage,eduYn,meta";

function csvOf(...dataRows: string[]): string {
  return [VALID_HEADER, ...dataRows].join("\n");
}

const VALID_ROW = "GN-01,강남 풋살장,강남구,INDOOR,서울특별시 강남구 테헤란로 1,37.5,127.0,true,02-1234-5678,https://example.com,false,{}";

describe("[U-01] 유효 행 파싱", () => {
...
describe("[U-04] 필수 필드 누락 에러", () => {
  it("code가 빈 문자열이면 errors에 포함된다", () => {
    const row = ",강남 풋살장,강남구,INDOOR,서울특별시 강남구,37.5,127.0,true,02-1234-5678,,false,";
  ...
  it("tel이 빈 문자열이면 errors에 포함된다", () => {
    const row = "GN-01,강남 풋살장,강남구,INDOOR,서울특별시 강남구,37.5,127.0,true,,, false,";
    const result = parseCsvFacilities(csvOf(row));
    expect(result.errors).toHaveLength(1);
    expect(result.errors[0]?.errors).toEqual(
...
describe("[U-05] type enum 검증", () => {
  it("type이 INDOOR|OUTDOOR|MIXED가 아니면 errors에 포함된다", () => {
    const row = "GN-01,강남 풋살장,강남구,UNKNOWN,서울특별시 강남구,37.5,127.0,true,02-1234-5678,,false,";
    const result = parseCsvFacilities(csvOf(row));
    expect(result.errors).toHaveLength(1);
    expect(result.errors[0]?.errors).toEqual(
  ...
  it("type은 소문자 입력도 대문자로 정규화해 유효 처리한다", () => {
    const row = "GN-01,강남 풋살장,강남구,outdoor,서울특별시 강남구,37.5,127.0,true,02-1234-5678,,false,";
    const result = parseCsvFacilities(csvOf(row));
    expect(result.valid).toHaveLength(1);
...
describe("[U-06] 위경도 numeric 검증", () => {
  it("lat이 비숫자이면 errors에 포함된다", () => {
    const row = "GN-01,강남 풋살장,강남구,INDOOR,서울특별시 강남구,abc,127.0,true,02-1234-5678,,false,";
    const result = parseCsvFacilities(csvOf(row));
    expect(result.errors).toHaveLength(1);
    expect(result.errors[0]?.errors).toEqual(
  ...
  it("lng가 빈 문자열이면 errors에 포함된다", () => {
    const row = "GN-01,강남 풋살장,강남구,INDOOR,서울특별시 강남구,37.5,,true,02-1234-5678,,false,";
    const result = parseCsvFacilities(csvOf(row));
    expect(result.errors).toHaveLength(1);
    expect(result.errors[0]?.errors).toEqual(
  ...
  it("유효한 lat/lng는 location 'lat,lng' 문자열로 변환된다", () => {
    const row = "GN-01,강남 풋살장,강남구,MIXED,서울특별시 강남구,37.123,127.456,false,02-9999-9999,,true,";
    const result = parseCsvFacilities(csvOf(row));
    expect(result.valid[0]?.location).toBe("37.123,127.456");
  });
});

describe("[U-07] parking / eduYn boolean 파싱", () => {
  it.each([
    ["true", true],
    ["1", true],
    ["y", true],
    ...
    const row = `GN-01,강남 풋살장,강남구,INDOOR,서울특별시 강남구,37.5,127.0,${parkingRaw},02-1234-5678,,false,`;
    const result = parseCsvFacilities(csvOf(row));
    expect(result.valid[0]?.parking).toBe(expected);
  });
});

describe("[U-08] 큰따옴표 감싸인 필드 (RFC 4180 서브셋)", () => {
  it("큰따옴표로 감싸인 필드 내 쉼표는 필드 구분자로 처리되지 않는다", () => {
    const row = `GN-01,강남 풋살장,강남구,INDOOR,"서울특별시, 강남구 테헤란로",37.5,127.0,true,02-1234-5678,,false,`;
    const result = parseCsvFacilities(csvOf(row));
    expect(result.valid).toHaveLength(1);
...
describe("[U-09] 다중 행 — 유효/에러 혼합", () => {
  it("유효 행과 에러 행이 섞인 경우 각 배열에 올바르게 분리된다", () => {
    const validRow = "GN-01,강남 풋살장,강남구,INDOOR,서울특별시 강남구,37.5,127.0,true,02-1234-5678,,false,";
    const errorRow = ",강북 풋살장,강북구,INDOOR,서울특별시 강북구,37.6,127.1,false,02-5555-5555,,true,";
...
describe("[U-10] homePage / meta 선택 필드", () => {
  it("homePage와 meta가 빈 문자열이면 undefined로 처리된다", () => {
    const row = "GN-01,강남 풋살장,강남구,OUTDOOR,서울특별시 강남구,37.5,127.0,false,02-1234-5678,,false,";
    const result = parseCsvFacilities(csvOf(row));
