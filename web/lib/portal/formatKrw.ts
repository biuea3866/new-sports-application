 ...
 *
 * `app/portal/insights/`에 국한돼 있던 유틸을 `lib/portal/`로 옮겨 매출(`/portal/payments`)
 * 화면과 공유한다 — 원화를 표시하는 모든 화면이 같은 반올림·자릿수 규칙을 따라야
 * `sellerAmount.toLocaleString("ko-KR")`처럼 기본값(소수 3자리)에 열린 표시가 재발하지 않는다.
 */
