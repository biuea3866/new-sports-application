...
- **Aggregate 고아**: 루트를 soft-delete/취소하는 DomainService 가 자식을 전파(soft-delete/취소)하지 않음 → 자식이 `deleted_at IS NULL` 로 생존 (Post→Comment, TicketOrder→Ticket, Event→Seat, Order→Item). 전파 메서드를 정의만 하고 호출 안 하는 데드 메서드 포함
- **RepositoryImpl 비즈니스 로직**: `*RepositoryImpl.kt` 가 중복 해소·`.softDelete()` 호출·상태 결정·활성마커 관리 등 도메인 로직 수행 (조회/위임만 해야 함 — 로직은 DomainService)
- **UseCase 결제 동기 분기**: `when (payment.status)` / `if (payment.status == ...)` 로 confirm/cancel 분기 (prepare 직후 항상 READY → 정상 흐름 깨짐, 완료는 웹훅/이벤트 비동기로)
- **capacity/좌석/재고 DB 최종 방어선 부재**: 분산락·낙관락만 있고 DB unique 제약이 없어 락 만료 시 오버부킹/이중발권 가능
- **낙관락 미처리**: `@Version` 충돌(`ObjectOptimisticLockingFailureException`)을 재시도/도메인예외(409)로 처리 안 하고 500 노출
- **트랜잭션 내 외부 호출**: `@Transactional` 안에서 PG·SMS·푸시 외부 Gateway 호출 / 도메인 이벤트를 커밋 전 발행 (AFTER_COMMIT 아님)
- **OSIV 위반**: `@Transactional` 메서드(UseCase·DomainService)가 JPA Entity 를 반환 (트랜잭션 밖 lazy 접근 위험 → DTO/primitive/Unit 반환) / `spring.jpa.open-in-view=true` 로 가림
