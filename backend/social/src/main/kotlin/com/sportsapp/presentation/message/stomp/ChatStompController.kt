...
import com.sportsapp.application.message.usecase.SendTypingUseCase
 ...
 * `/app/rooms/{roomId}/typing` 은 영속화가 필요 없는 실시간 신호라 [SendTypingUseCase] 를 경유해 즉시 팬아웃한다.
 * 두 엔드포인트 모두 presentation → application(UseCase) → domain(MessageDomainService) → Gateway
 * 순서를 지키며, Gateway 는 UseCase 에 직접 노출되지 않는다.
 *
...
class ChatStompController(
    ...
    fun typing(
        ...
        sendTypingUseCase.execute(request.toCommand(roomId, principal.userId()))
    }
...
data class StompTypingRequest(val typing: Boolean) {
    fun toCommand(roomId: Long, userId: Long): BroadcastTypingCommand =
        BroadcastTypingCommand(roomId = roomId, userId = userId, typing = typing)
}
