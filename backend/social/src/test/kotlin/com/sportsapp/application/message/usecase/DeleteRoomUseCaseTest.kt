...
class DeleteRoomUseCaseTest : BehaviorSpec({
    ...
    val deleteRoomUseCase = DeleteRoomUseCase(roomDomainService)

    Given("본인이 참가자인 룸") {
        justRun { roomDomainService.leaveRoom(10L, 1L) }

        When("execute 를 호출하면") {
            deleteRoomUseCase.execute(roomId = 10L, userId = 1L)

            Then("leaveRoom 이 호출된다") {
                verify { roomDomainService.leaveRoom(10L, 1L) }
            }
        }
    }

    Given("본인이 참가자가 아닌 룸") {
        every { roomDomainService.leaveRoom(20L, 99L) } throws NotRoomParticipantException(
            userId = 99L,
            roomId = 20L,
        )
    ...
    Given("마지막 참가자가 탈퇴하는 룸") {
        justRun { roomDomainService.leaveRoom(30L, 1L) }
        ...
        When("execute 를 호출하면 (내부에서 마지막 1인 탈퇴 처리)") {
            ...
            Then("[U-03] leaveRoom 이 호출되고 DomainService 가 Room+Message 삭제를 담당한다") {
                verify { roomDomainService.leaveRoom(30L, 1L) }
            }
        }
    }
})
