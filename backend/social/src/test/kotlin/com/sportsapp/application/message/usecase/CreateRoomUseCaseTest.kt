...
class CreateRoomUseCaseTest : BehaviorSpec({
    ...
    val createRoomUseCase = CreateRoomUseCase(roomDomainService)

    Given("1:1 룸 생성 요청 — 기존 룸이 없는 경우") {
        val newRoom = Room.createDirect()
        every { roomDomainService.createOrFindOneToOne(1L, 2L) } returns newRoom
        ...
        When("participantIds=[1,2], name=null 로 execute 를 호출하면") {
            ...
            Then("[U-01] 새 룸이 반환된다") {
                ...
                verify { roomDomainService.createOrFindOneToOne(1L, 2L) }
            }
        }
    }
    ...
    Given("1:1 룸 생성 요청 — 기존 룸이 있는 경우") {
        ...
        every { roomDomainService.createOrFindOneToOne(3L, 4L) } returns existingRoom
        ...
        When("동일 participantIds 로 두 번 호출하면") {
            ...
            Then("[U-01] 동일 룸이 반환되고 createOrFindOneToOne 이 2회 호출된다") {
                ...
                verify(exactly = 2) { roomDomainService.createOrFindOneToOne(3L, 4L) }
            }
        }
    }
    ...
    Given("그룹 룸 생성 요청 — 호출자가 participantIds에 포함된 경우") {
        ...
        every {
            roomDomainService.createGroupRoom("축구 모임", listOf(1L, 2L, 3L), hostUserId = 1L)
        } returns groupRoom
        ...
        When("requestUserId=1, participantIds=[1,2,3], name='축구 모임' 으로 execute 를 호출하면") {
            ...
            Then("[U-02] 그룹 룸이 반환되고 참여자 3명이 등록된다") {
                ...
                verify { roomDomainService.createGroupRoom("축구 모임", listOf(1L, 2L, 3L), hostUserId = 1L) }
            }
    ...
    Given("그룹 룸 생성 요청 — 호출자가 participantIds에 미포함된 경우") {
        ...
        every {
            roomDomainService.createGroupRoom("농구 모임", listOf(1L, 2L, 3L), hostUserId = 1L)
        } returns groupRoom
        ...
        When("requestUserId=1, participantIds=[2,3], name='농구 모임' 으로 execute 를 호출하면") {
            ...
            Then("[U-03] 호출자가 자동 추가되어 참여자 3명으로 그룹 룸이 생성된다") {
                result.type shouldBe RoomType.GROUP
                verify { roomDomainService.createGroupRoom("농구 모임", listOf(1L, 2L, 3L), hostUserId = 1L) }
            }
        }
    }
