 ...
 * 형제 공급자 `InternalProgramCatalogCriteriaTest`(S2-04)와 같은 규약을 고정한다.
 */
class InternalRecruitmentCatalogCriteriaTest : BehaviorSpec({

    Given("파사드가 요청한 넓은 창(page=0, size=300)") {
        val criteria = InternalRecruitmentCatalogCriteria(keyword = null, page = 0, size = 300)
    ...
    Given("일반적인 단일 페이지 요청(page=2, size=20)") {
        val criteria = InternalRecruitmentCatalogCriteria(keyword = "축구", page = 2, size = 20)
    ...
    Given("컨트롤러 기본값 상수") {
        Then("@RequestParam 이 쓰는 문자열 형태로 한 곳에만 정의된다 (매직값 이원화 방지)") {
            InternalRecruitmentCatalogCriteria.DEFAULT_PAGE_PARAM shouldBe "0"
            InternalRecruitmentCatalogCriteria.DEFAULT_SIZE_PARAM shouldBe "20"
        }
    }
})
