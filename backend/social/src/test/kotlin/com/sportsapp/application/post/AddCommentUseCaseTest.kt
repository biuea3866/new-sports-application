...
class AddCommentUseCaseTest : BehaviorSpec({
    ...
    Given("전역 Post에 댓글 작성 시") {
        ...
        every { fixture.postDomainService.getPost(1L) } returns post
        every { fixture.commentDomainService.addComment(postId = 1L, userId = 10L, content = "댓글 내용") } returns comment

        When("execute를 호출하면") {
            val result = fixture.useCase.execute(AddCommentCommand(postId = 1L, userId = 10L, content = "댓글 내용"))
            ...
            Then("CommentResponse가 반환되고 community 인가는 호출되지 않는다") {
                ...
                verify(exactly = 0) { fixture.communityDomainService.requireActiveMember(any(), any()) }
            }
        }
    }
    ...
    Given("미존재 Post에 댓글 작성 시") {
        val fixture = newUseCase()
        every { fixture.postDomainService.getPost(99999L) } throws ResourceNotFoundException("Post", 99999L)

        When("execute를 호출하면") {
            Then("ResourceNotFoundException을 던진다") {
                shouldThrow<ResourceNotFoundException> {
                    fixture.useCase.execute(AddCommentCommand(postId = 99999L, userId = 10L, content = "댓글"))
                }
            }
        }
    }
    ...
    Given("PRIVATE 모임 소속 Post에 ACTIVE 멤버가 댓글을 작성하면") {
        ...
        every { fixture.postDomainService.getPost(2L) } returns post
        every { fixture.communityDomainService.requireActiveMember(20L, 5L) } just Runs
        every { fixture.commentDomainService.addComment(postId = 2L, userId = 5L, content = "댓글") } returns comment

        When("execute를 호출하면") {
            val result = fixture.useCase.execute(AddCommentCommand(postId = 2L, userId = 5L, content = "댓글"))

            Then("댓글이 정상 작성된다") {
                result.userId shouldBe 5L
                verify(exactly = 1) { fixture.communityDomainService.requireActiveMember(20L, 5L) }
            }
        }
    }
    ...
    Given("PRIVATE 모임 소속 Post에 비멤버가 댓글을 작성하면") {
        ...
        every { fixture.postDomainService.getPost(3L) } returns post
        every { fixture.communityDomainService.requireActiveMember(20L, 6L) } throws NotCommunityMemberException(20L, 6L)

        Then("NotCommunityMemberException 을 던진다") {
            shouldThrow<NotCommunityMemberException> {
                fixture.useCase.execute(AddCommentCommand(postId = 3L, userId = 6L, content = "댓글"))
            }
        }
    }
})
