...
-- 백필: 기존 GROUP/컨텍스트 방(type <> 'DIRECT')의 host_user_id 를 "활성(deleted_at IS NULL) MEMBER
--   중 joined_at 최솟값 참여자의 user_id"로 채운다 — 기존 GuestInvitationDomainService.requireHost
--   규칙과 동일해 기존 초대 권한이 변하지 않는다. 동일 joined_at 이 여럿이면 user_id 최솟값으로 결정성을
--   부여한다(기존 Kotlin `minByOrNull`도 동률 시 순서가 임의였으므로 동작 의미상 문제 없음).
--   DIRECT(1:1) 방은 명시적으로 백필 대상에서 제외해 host_user_id 를 NULL로 남긴다 — `Room.createDirect()`가
--   신규 DIRECT 방을 항상 hostUserId=null 로 만드는 것과 일치시켜야, 기존/신규 DIRECT 방이 동일하게
--   호스트 없음으로 취급되고 게스트 초대·방출 같은 방장 전용 행위가 우연히 열리지 않는다(로컬 MySQL
--   8.0.46 재현: 필터 없이 백필 시 DIRECT 방에도 첫 참여자가 host로 채워짐을 확인). MEMBER 참여자가
--   없는 방(전원 GUEST 등)도 host_user_id 가 NULL로 남는다 — host 액션(초대/방출) 비대상이라 문제 없음.
