...
/**
 * MCP Read tool — 시설 구/유형별, 시도·시군구/유형별 통계 조회.
...
class McpFacilityStatsTools(
    ...
    @PreAuthorize("@authz.hasMcpScope('read:facility:stats')")
    @Tool(
        name = "getRegionTypeStats",
        description = "시도·시군구 표준코드와 시설 유형(type)별 시설 수 통계를 조회합니다. 필터 없이 전체 집계 결과를 반환합니다.",
    )
    fun getRegionTypeStats(): McpResponse<List<RegionTypeCountResponse>> =
        mcpAuditLogAsyncRecorder.withAudit("getRegionTypeStats", emptyMap()) {
            val stats = getRegionTypeStatsUseCase.execute().map { RegionTypeCountResponse.of(it) }
            McpResponse.ok(data = stats)
        }
}
