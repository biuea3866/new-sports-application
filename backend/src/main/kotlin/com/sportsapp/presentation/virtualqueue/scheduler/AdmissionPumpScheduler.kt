...
/**
 * 가상 대기열 Admission Pump — 활성 대상(`VirtualQueueStore.activeTargets`)을 틱(2초, BE-01
 * `virtual-queue.admission.tick-seconds`)마다 순회해 배치 admission 전진·이탈 방출을 트리거한다
 * (BE-07, TDD "서버 토폴로지"·redis-contract §3).
 *
 ...
 *
 * 롤백: `virtual-queue.admission.enabled=false`로 배치 실행 자체를 즉시 중단할 수 있다(운영 킬 스위치).
 */
@Component
class AdmissionPumpScheduler(
    ...
    @Value("\${virtual-queue.admission.enabled:true}") private val admissionEnabled: Boolean,
    ...
    fun pump() {
        if (!admissionEnabled) {
            log.info("AdmissionPumpScheduler: disabled by virtual-queue.admission.enabled flag, skipping")
            return
        }
        virtualQueueStore.activeTargets().forEach(::pumpTarget)
    }
