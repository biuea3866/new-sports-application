...
class PopularProductsRedisRepository(
    ...
    private val productListType = object : TypeReference<List<PopularProductCacheDto>>() {}

    override fun get(category: ProductCategory): List<Product>? {
        val json = stringRedisTemplate.opsForValue().get(cacheKey(category)) ?: return null
        return objectMapper.readValue(json, productListType).map { it.toProduct() }
    }

    override fun put(category: ProductCategory, products: List<Product>) {
        val json = objectMapper.writeValueAsString(products.map { PopularProductCacheDto.of(it) })
