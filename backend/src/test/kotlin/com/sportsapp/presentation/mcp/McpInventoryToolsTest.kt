...
class McpInventoryToolsTest : BehaviorSpec({
    ...
    val mcpInventoryTools = McpInventoryTools(getInventoryUseCase, mcpAuditLogAsyncRecorder)
    ...
    Given("getInventory tool") {
        ...
        When("[U-08] IDOR — principal이 없으면 (SecurityContext 비어있음)") {
            ...
            val localTools = McpInventoryTools(localUseCase, localRecorder)
        ...
        When("[U-audit-05] getInventory 호출 시 audit recorder가 1회 호출된다") {
            ...
            Then("[U-audit-05] mcpAuditLogAsyncRecorder.record가 정확히 1회 호출된다") {
                verify(exactly = 1) {
