...
class BE10PostCommentOrphanTest(
    ...
    init {
        ...
        Given("[R-01] findAllActiveByPostId로 가져온 댓글을 softDelete 후 saveAll하면 findByPostId는 0건을 반환한다") {
            ...
            commentRepository.save(Comment.create(post = post, userId = 2L, content = "댓글1"))
            commentRepository.save(Comment.create(post = post, userId = 3L, content = "댓글2"))
        ...
        Given("[R-02] softDelete는 다른 postId의 Comment에 영향을 주지 않는다") {
            ...
            commentRepository.save(Comment.create(post = postA, userId = 2L, content = "A댓글"))
            commentRepository.save(Comment.create(post = postB, userId = 2L, content = "B댓글"))
        ...
        Given("[S-01][S-02] deletePost 후 해당 Post의 모든 Comment가 soft-delete된다 (루트 soft-delete → 자식 조회 0건)") {
            ...
            commentRepository.save(Comment.create(post = post, userId = 2L, content = "댓글A"))
            commentRepository.save(Comment.create(post = post, userId = 3L, content = "댓글B"))
