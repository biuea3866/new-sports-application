...
class GetMyCartUseCase(
    ...
    // 최초 접근 시 cart 동시 생성 경합: UNIQUE 충돌 시 fresh tx 재시도 → 기존 cart 재조회로 수렴.
    @Retryable(
    ...
    @Transactional
    fun execute(userId: Long): Pair<Cart, List<CartLineItem>> =
        cartDomainService.getCartWithLineItems(userId)
}
