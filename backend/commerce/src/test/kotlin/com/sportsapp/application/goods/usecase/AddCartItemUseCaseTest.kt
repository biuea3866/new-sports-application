...
class AddCartItemUseCaseTest : BehaviorSpec({
    ...
    Given("유효한 AddCartItemCommand가 주어졌을 때") {
        ...
        val cartLineItem = CartLineItem(
            ...
            productImageUrl = "https://cdn.example.com/10.jpg",
            unitPrice = BigDecimal("119000"),
            quantity = 2,
        )
        ...
        every {
            cartDomainService.addItem(command.userId, command.productId, command.quantity)
        } returns (cart to listOf(cartLineItem))

        When("execute를 호출하면") {
            Then("[U-happy] Cart와 장바구니 항목 목록이 반환된다") {
                ...
                items[0].productName shouldBe "카본 배드민턴 라켓"
            }
        }
    }
})
