...
/**
 * W1-11b — [TicketOrderExpiryProperties]는 빠른 TTL(ttlMinutes)·느린 TTL(readyTtlMinutes) 두
 * 값을 갖는다. readyTtlMinutes가 ttlMinutes보다 크지 않으면 결제 진행 중(live)인 주문도
 * 빠른 TTL에 의해 오만료될 수 있으므로, 이 불변조건을 `require`로 강제해 부팅 시점에
 * 실패시킨다.
 *
 * 생성자 직접 호출 테스트만으로는 이 불변조건이 실제로 Spring 부팅을 막는지 검증하지
 * 못한다 — booking(W1-11c) [BookingExpiryPropertiesTest] 선례와 동일하게
 * [ApplicationContextRunner] + `@EnableConfigurationProperties`로 컨텍스트 기동 자체와 env
 * 재정의 반영 여부를 함께 검증한다.
 */
@Configuration
@EnableConfigurationProperties(TicketOrderExpiryProperties::class)
class TicketOrderExpiryPropertiesTestConfig

class TicketOrderExpiryPropertiesTest : BehaviorSpec({

    fun contextRunner() = ApplicationContextRunner()
        .withUserConfiguration(TicketOrderExpiryPropertiesTestConfig::class.java)

    Given("readyTtlMinutes가 ttlMinutes보다 클 때") {
        When("생성하면") {
            val properties = TicketOrderExpiryProperties(ttlMinutes = 15, readyTtlMinutes = 60)
        ...
        When("생성하면") {
            Then("IllegalArgumentException을 던져 부팅을 실패시킨다") {
                shouldThrow<IllegalArgumentException> {
                    TicketOrderExpiryProperties(ttlMinutes = 15, readyTtlMinutes = 15)
                }
            }
        }
    }
    ...
    Given("readyTtlMinutes가 ttlMinutes보다 작을 때 (핵심 회귀 — 가드 무력화 방지)") {
        When("생성하면") {
            Then("IllegalArgumentException을 던져 부팅을 실패시킨다") {
                shouldThrow<IllegalArgumentException> {
                    TicketOrderExpiryProperties(ttlMinutes = 15, readyTtlMinutes = 10)
                }
            }
        }
    }
    ...
    Given("env로 readyTtlMinutes<=ttlMinutes를 주입한 경우 (불변조건 위반으로 컨텍스트 기동이 실패한다)") {
        When("ApplicationContextRunner로 컨텍스트를 로드하면") {
            Then("빈 생성이 실패해 컨텍스트 기동 자체가 실패한다") {
                contextRunner()
                    .withPropertyValues(
                        "ticketing.expiry.ttl-minutes=15",
                        "ticketing.expiry.ready-ttl-minutes=10",
                    )
                    .run { context ->
            ...
            Then("실패 원인 체인에 IllegalArgumentException(불변조건 위반)이 포함된다") {
                contextRunner()
                    .withPropertyValues(
                        "ticketing.expiry.ttl-minutes=15",
                        "ticketing.expiry.ready-ttl-minutes=15",
                    )
                    .run { context ->
        ...
        When("ApplicationContextRunner로 컨텍스트를 로드하면") {
            Then("재정의한 값이 빈에 그대로 반영된다 — JavaBean 바인딩으로 떨어져 조용히 무시되지 않는다") {
                contextRunner()
                    .withPropertyValues(
                        "ticketing.expiry.ttl-minutes=15",
                        "ticketing.expiry.ready-ttl-minutes=120",
                    )
                    .run { context ->
                        val properties = context.getBean(TicketOrderExpiryProperties::class.java)
                        properties.ttlMinutes shouldBe 15
