...
| 단계 | 값 | 비고 |
|---|---|---|
| 1 | `HS256` | 기본값. **공개키 미주입 상태에서도 부팅은 정상이지만, 이는 1단계에 한해서만 안전하다 — 2단계 진입 전에는 반드시 6개 서비스 전부에 공개키를 주입해야 한다** (RS256 검증 비활성, HS256만 검증) |
| 2 | `RS256` | **진입 게이트(필수, 아래 참고)** 통과 후 1단계 무사고 확인 → platform에서만 전환 |
| 3 | `RS256` (검증도 RS256 단독) | 기존 HS256 토큰 최대 유효기간(액세스 토큰 30분) 경과 후, `JwtTokenProvider.parseClaims`의 HS256 fallback 코드를 제거 |

### 2단계 진입 게이트 (필수 — platform을 RS256으로 올리기 전에 확인)

플랫폼 서비스만 `APP_JWT_ALGORITHM=RS256`으로 올리면 그 순간부터 RS256 서명 토큰이 발급된다.
**나머지 5개 서비스에 `APP_JWT_PUBLIC_KEY`가 주입돼 있지 않으면, 그 서비스들은 RS256 검증을
시도조차 하지 않고(공개키 없으면 RS256 경로 비활성) HS 서명과도 일치하지 않아 검증에 실패한다 —
전 서비스 401 대량 발생으로 이어진다.**

따라서 2단계(`APP_JWT_ALGORITHM=RS256` 적용)로 넘어가기 전, 아래를 **반드시** 확인한다.

1. 6개 서비스 전부의 실행 환경(compose `environment:` 또는 `.env`)에 `APP_JWT_PUBLIC_KEY`가
   설정돼 있는지 확인한다 — 예: `docker compose exec <service> env | grep APP_JWT_PUBLIC_KEY`로
   6개 서비스 각각에서 값이 비어 있지 않은지 눈으로 확인한다.
2. 5개 서비스(platform 제외)를 공개키 주입 상태로 먼저 재기동하고, **HS256 검증이 그대로
   동작하는지**(RS256 미전환 상태) 확인한다 — 공개키 주입 자체는 검증 경로를 바꾸지 않는다.
3. 위 1·2가 모두 확인된 뒤에만 platform의 `APP_JWT_ALGORITHM`을 `RS256`으로 전환한다.

이 게이트를 건너뛰고 platform만 먼저 전환하면 위 401 대량 발생 시나리오가 그대로 재현된다.
