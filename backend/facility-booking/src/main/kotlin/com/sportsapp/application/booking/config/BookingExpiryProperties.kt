 ...
 * 값이라 재기동 전제가 무방하다.
 *
 * **두 TTL (4차 재설계)**: payment 상태만으로 "결제가 시작이라도 됐는가"(live)를 판정하고,
 * live 여부에 따라 서로 다른 TTL을 적용한다([com.sportsapp.domain.payment.service.PaymentLivenessClassifier]
 * 참고) — 활동 시각(updatedAt) 기반 판정은 initiatePg가 주문 생성 요청 트랜잭션 안에서 끝나
 * 신호가 되지 못해 3차 설계에서 폐기됐다.
 * - [ttlMinutes](빠른 TTL): 결제 미시작/실패/취소/환불 등 "결제가 시작 못 했거나 종결된" 예약을
 *   만료시키는 기준. F-A(고립 예약)를 정확히 타격한다.
 * - [readyTtlMinutes](느린 TTL): live(READY 이상) 결제가 있는 예약을 만료시키는 기준. 결제창에
 *   머무는 사용자를 오만료하지 않도록 충분히 길게 잡는다.
 *
 * **불변조건**: `readyTtlMinutes`는 반드시 `ttlMinutes`보다 커야 한다 — 같거나 짧으면 live
 * 예약도 빠른 TTL에 걸려 오만료된다. env 재정의로도 이 조건이 깨지면 부팅을 실패시켜
...
data class BookingExpiryProperties(
    ...
    companion object {
        /**
         * 스케줄 주기(ms) 기본값 SSOT — [com.sportsapp.presentation.booking.scheduler.BookingExpiryScheduler]의
         * `@Scheduled(fixedDelayString)` 애노테이션이 컴파일 타임 상수로 이 값을 문자열 보간해 참조한다
         * (p3-4: 매직 상수가 properties 밖에 흩어지는 것 방지).
         */
