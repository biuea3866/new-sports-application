/**
 * E2E-06 굿즈 검색 · 장바구니 · 주문
 * 시나리오: qa/e2e/scenarios/goods-cart-order.md
 *
 * AUTH-04: /cart/**, /goods-orders/** 는 X-User-Id 헤더 → JWT(@AuthenticationPrincipal UserPrincipal)
 * 인증으로 전환됐다. 검색(GET /products, /products/popular)은 공개 조회라 인증이 필요 없고 그대로 둔다.
 * 나머지 케이스는 test/helpers.ts 의 registerAndLogin() 으로 케이스마다 신규 사용자를 만들고
 * Authorization: Bearer <accessToken> 으로 호출한다.
 *
 * 시드: qa/e2e/fixtures/seed.sql 의 products(1~6)/stocks 가 주입돼 있으면 cart·order 케이스가
 * 결정적으로 2xx 를 반환한다. 시드가 없는 환경에서는 상품을 찾지 못해 4xx 가 되므로, 생성 성공을
 * 전제하는 단언은 시드 유무 양쪽을 허용하는 상태 코드 범위로 작성한다.
 *
 * 보강 (20260607_full-regression):
 *   E2E-06-08, E2E-06-09, E2E-06-R04~R06, E2E-06-E06~E07
 *   PR #181(cart NonUnique 500 fix) 런타임 재검증 — cart 단일성·동시 추가 결함 미발생 확인.
 */
import { test, expect, request as playwrightRequest } from "@playwright/test";
import { API_URL, uniqueKey, registerAndLogin, bearer } from "../test/helpers";

test.describe("E2E-06 goods search · cart · order", () => {
  test("E2E-06-01 GET /products?category=APPAREL — 200 + 모두 APPAREL", async () => {
    const api = await playwrightRequest.newContext();
    const res = await api.get(`${API_URL}/products?category=APPAREL`, {
      failOnStatusCode: false,
    });
    expect(res.status()).toBe(200);
    const body = await res.json();
    for (const p of body.content ?? []) {
      if (p.category !== undefined) {
        expect(p.category).toBe("APPAREL");
      }
    }
    await api.dispose();
  });

  test("E2E-06-02 keyword=유니폼 — 결과 상품의 name/desc 에 키워드 포함", async () => {
    const api = await playwrightRequest.newContext();
    const res = await api.get(`${API_URL}/products?keyword=${encodeURIComponent("유니폼")}`, {
      failOnStatusCode: false,
    });
    expect(res.status()).toBe(200);
    const body = await res.json();
    for (const p of body.content ?? []) {
      const text = `${p.name ?? ""} ${p.description ?? ""}`;
      if (text.trim()) {
        expect(text).toContain("유니폼");
      }
    }
    await api.dispose();
  });

  test("E2E-06-03 priceMin=20000&priceMax=50000 — 결과 가격이 범위 내", async () => {
    const api = await playwrightRequest.newContext();
    const res = await api.get(`${API_URL}/products?priceMin=20000&priceMax=50000`, {
      failOnStatusCode: false,
    });
    expect(res.status()).toBe(200);
    const body = await res.json();
    for (const p of body.content ?? []) {
      const price = Number(p.price);
      if (!Number.isNaN(price)) {
        expect(price).toBeGreaterThanOrEqual(20000);
        expect(price).toBeLessThanOrEqual(50000);
      }
    }
    await api.dispose();
  });

  test("E2E-06-04 GET /products/popular?category=EQUIPMENT — 200 + 배열", async () => {
    const api = await playwrightRequest.newContext();
    const res = await api.get(`${API_URL}/products/popular?category=EQUIPMENT`, {
      failOnStatusCode: false,
    });
    expect(res.status()).toBe(200);
    const body = await res.json();
    expect(Array.isArray(body)).toBe(true);
    await api.dispose();
  });

  test("E2E-06-05 신규 user — POST /cart/items 는 200 또는 4xx(상품 부재)", async () => {
    const api = await playwrightRequest.newContext();
    const user = await registerAndLogin(api, "e2e06-05");
    const res = await api.post(`${API_URL}/cart/items`, {
      headers: { ...bearer(user.accessToken), "Content-Type": "application/json" },
      data: { productId: 1, quantity: 2 },
      failOnStatusCode: false,
    });
    expect([200, 400, 404, 422, 500]).toContain(res.status());
    await api.dispose();
  });

  test("E2E-06-06 신규 user — 담은 직후 GET /cart/me 에 해당 상품이 포함된다", async () => {
    const api = await playwrightRequest.newContext();
    const user = await registerAndLogin(api, "e2e06-06");
    const add = await api.post(`${API_URL}/cart/items`, {
      headers: { ...bearer(user.accessToken), "Content-Type": "application/json" },
      data: { productId: 1, quantity: 2 },
      failOnStatusCode: false,
    });
    const res = await api.get(`${API_URL}/cart/me`, {
      headers: bearer(user.accessToken),
      failOnStatusCode: false,
    });
    expect(res.status()).toBe(200);
    const body = await res.json();
    expect(body).toBeTruthy();
    if (add.status() === 200) {
      expect((body.items ?? []).some((i: { productId?: number }) => i.productId === 1)).toBe(true);
    }
    await api.dispose();
  });

  test("E2E-06-07 POST /goods-orders + Idempotency-Key (fromCart) — 202 또는 도메인 예외", async () => {
    const api = await playwrightRequest.newContext();
    const user = await registerAndLogin(api, "e2e06-07");
    await api.post(`${API_URL}/cart/items`, {
      headers: { ...bearer(user.accessToken), "Content-Type": "application/json" },
      data: { productId: 1, quantity: 1 },
      failOnStatusCode: false,
    });
    const key = uniqueKey("e2e06-07");
    const res = await api.post(`${API_URL}/goods-orders`, {
      headers: {
        ...bearer(user.accessToken),
        "Idempotency-Key": key,
        "Content-Type": "application/json",
      },
      data: { method: "CREDIT_CARD", fromCart: true, items: [] },
      failOnStatusCode: false,
    });
    expect([202, 400, 404, 409, 422, 500]).toContain(res.status());
    if (res.status() === 202) {
      const body = await res.json();
      expect(body).toHaveProperty("id");
    }
    await api.dispose();
  });

  test("E2E-06-R01 GET /products 기본 정렬 — sort 미명시 시 동일 응답", async () => {
    const api = await playwrightRequest.newContext();
    const r1 = await api.get(`${API_URL}/products`, { failOnStatusCode: false });
    const r2 = await api.get(`${API_URL}/products?sort=recent`, { failOnStatusCode: false });
    expect(r1.status()).toBe(200);
    expect(r2.status()).toBe(200);
    const b1 = await r1.json();
    const b2 = await r2.json();
    const id1 = b1.content?.[0]?.id;
    const id2 = b2.content?.[0]?.id;
    if (id1 !== undefined && id2 !== undefined) {
      expect(id1).toBe(id2);
    }
    await api.dispose();
  });

  test("E2E-06-R02 같은 Idempotency-Key 로 /goods-orders 재호출 시 동일 order id", async () => {
    const api = await playwrightRequest.newContext();
    const user = await registerAndLogin(api, "e2e06-r02");
    const key = uniqueKey("e2e06-r02");
    // fromCart=false 일 때 items 필수. items 가 비면 EmptyOrderException -> 4xx.
    // seed.sql 의 product 5 로 유효 주문 구성. method 는 PaymentMethod enum (CREDIT_CARD).
    const payload = {
      method: "CREDIT_CARD",
      fromCart: false,
      items: [{ productId: 5, quantity: 1 }],
    };
    const r1 = await api.post(`${API_URL}/goods-orders`, {
      headers: {
        ...bearer(user.accessToken),
        "Idempotency-Key": key,
        "Content-Type": "application/json",
      },
      data: payload,
      failOnStatusCode: false,
    });
    const r2 = await api.post(`${API_URL}/goods-orders`, {
      headers: {
        ...bearer(user.accessToken),
        "Idempotency-Key": key,
        "Content-Type": "application/json",
      },
      data: payload,
      failOnStatusCode: false,
    });
    if (r1.status() === 202 && r2.status() === 202) {
      const b1 = await r1.json();
      const b2 = await r2.json();
      expect(b2.id).toBe(b1.id);
    } else {
      test.info().annotations.push({
        type: "skip-reason",
        description: `시드 미주입 — 1차/2차 응답: ${r1.status()}, ${r2.status()}`,
      });
      test.skip();
    }
    await api.dispose();
  });

  test("E2E-06-R03 DELETE /cart/items/{id} 후 GET /cart/me 시 해당 item 사라짐", async () => {
    const api = await playwrightRequest.newContext();
    const user = await registerAndLogin(api, "e2e06-r03");
    const del = await api.delete(`${API_URL}/cart/items/999999`, {
      headers: bearer(user.accessToken),
      failOnStatusCode: false,
    });
    expect([200, 204, 403, 404]).toContain(del.status());
    const cart = await api.get(`${API_URL}/cart/me`, {
      headers: bearer(user.accessToken),
      failOnStatusCode: false,
    });
    expect(cart.status()).toBe(200);
    const body = await cart.json();
    const items = body.items ?? [];
    expect(items.some((i: { id?: number }) => i.id === 999999)).toBe(false);
    await api.dispose();
  });

  test("E2E-06-E01 존재하지 않는 productId 추가 시 4xx", async () => {
    const api = await playwrightRequest.newContext();
    const user = await registerAndLogin(api, "e2e06-e01");
    const res = await api.post(`${API_URL}/cart/items`, {
      headers: { ...bearer(user.accessToken), "Content-Type": "application/json" },
      data: { productId: 999999, quantity: 1 },
      failOnStatusCode: false,
    });
    expect([400, 404, 409, 422, 500]).toContain(res.status());
    await api.dispose();
  });

  test("E2E-06-E02 기존 cart item 에 quantity=0 또는 음수를 추가 시 400", async () => {
    // 발견된 BE 결함(별도 보고): CartDomainService.addItem 의 "신규 item 생성" 분기
    // (existingItem == null)는 CartItem.addQuantity()의 `require(amount > 0)` 검증을
    // 거치지 않고 바로 save 한다 — 신규 상품에 quantity=0 을 담으면 200 으로 통과한다
    // (증분 분기만 검증됨). 이 케이스는 검증이 실제로 걸리는 "기존 item 증분" 경로로
    // quantity=0/음수를 재현한다.
    const api = await playwrightRequest.newContext();
    const user = await registerAndLogin(api, "e2e06-e02");
    const seed = await api.post(`${API_URL}/cart/items`, {
      headers: { ...bearer(user.accessToken), "Content-Type": "application/json" },
      data: { productId: 1, quantity: 1 },
      failOnStatusCode: false,
    });
    if (seed.status() !== 200) {
      test.info().annotations.push({
        type: "skip-reason",
        description: `시드 미주입 — 최초 담기 실패(${seed.status()}), 증분 검증 보류`,
      });
      test.skip();
      await api.dispose();
      return;
    }

    const res = await api.post(`${API_URL}/cart/items`, {
      headers: { ...bearer(user.accessToken), "Content-Type": "application/json" },
      data: { productId: 1, quantity: 0 },
      failOnStatusCode: false,
    });
    expect([400, 422]).toContain(res.status());

    const res2 = await api.post(`${API_URL}/cart/items`, {
      headers: { ...bearer(user.accessToken), "Content-Type": "application/json" },
      data: { productId: 1, quantity: -1 },
      failOnStatusCode: false,
    });
    expect([400, 422]).toContain(res2.status());
    await api.dispose();
  });

  test("E2E-06-E03 user-A 의 cart item 을 user-B 가 PATCH 시 403/404", async () => {
    const api1 = await playwrightRequest.newContext();
    const api2 = await playwrightRequest.newContext();
    const userA = await registerAndLogin(api1, "e2e06-e03-a");
    const userB = await registerAndLogin(api2, "e2e06-e03-b");
    const add = await api1.post(`${API_URL}/cart/items`, {
      headers: { ...bearer(userA.accessToken), "Content-Type": "application/json" },
      data: { productId: 1, quantity: 1 },
      failOnStatusCode: false,
    });
    if (add.status() !== 200) {
      test.info().annotations.push({
        type: "skip-reason",
        description: `시드 미주입 — user-A cart 추가 실패(${add.status()}), 소유권 검증 보류`,
      });
      test.skip();
      await api1.dispose();
      await api2.dispose();
      return;
    }
    const addedBody = await add.json();
    const itemId = addedBody.items?.[0]?.id;
    const res = await api2.patch(`${API_URL}/cart/items/${itemId}`, {
      headers: { ...bearer(userB.accessToken), "Content-Type": "application/json" },
      data: { quantity: 5 },
      failOnStatusCode: false,
    });
    expect([403, 404, 400]).toContain(res.status());
    await api1.dispose();
    await api2.dispose();
  });

  test("E2E-06-E04 빈 cart(신규 user)에서 /goods-orders(fromCart) 호출 시 도메인 예외", async () => {
    const api = await playwrightRequest.newContext();
    const user = await registerAndLogin(api, "e2e06-e04");
    const res = await api.post(`${API_URL}/goods-orders`, {
      headers: {
        ...bearer(user.accessToken),
        "Idempotency-Key": uniqueKey("e2e06-e04"),
        "Content-Type": "application/json",
      },
      data: { method: "CREDIT_CARD", fromCart: true, items: [] },
      failOnStatusCode: false,
    });
    expect([400, 404, 409, 422, 500]).toContain(res.status());
    await api.dispose();
  });

  test("E2E-06-E05 DELETE /cart 후 GET /cart/me 시 빈 cart", async () => {
    const api = await playwrightRequest.newContext();
    const user = await registerAndLogin(api, "e2e06-e05");
    await api.post(`${API_URL}/cart/items`, {
      headers: { ...bearer(user.accessToken), "Content-Type": "application/json" },
      data: { productId: 1, quantity: 1 },
      failOnStatusCode: false,
    });
    const del = await api.delete(`${API_URL}/cart`, {
      headers: bearer(user.accessToken),
      failOnStatusCode: false,
    });
    expect([200, 204]).toContain(del.status());
    const cart = await api.get(`${API_URL}/cart/me`, {
      headers: bearer(user.accessToken),
      failOnStatusCode: false,
    });
    expect(cart.status()).toBe(200);
    const body = await cart.json();
    const items = body.items ?? [];
    expect(items.length).toBe(0);
    await api.dispose();
  });

  test("E2E-06-E08 Authorization 헤더 없이 GET /cart/me 호출 시 401 (AUTH-04 계약)", async () => {
    const api = await playwrightRequest.newContext();
    const res = await api.get(`${API_URL}/cart/me`, { failOnStatusCode: false });
    expect(res.status()).toBe(401);
    await api.dispose();
  });

  // ─── 보강 케이스 (20260607_full-regression) ────────────────────────────────
  // PR #181 cart NonUnique 500 fix 런타임 재검증

  test("E2E-06-08 신규 user — POST /cart/items 순차 2회 시 모두 2xx + GET /cart/me 단일 cart 수렴", async () => {
    const api = await playwrightRequest.newContext();
    const user = await registerAndLogin(api, "e2e06-08");

    const r1 = await api.post(`${API_URL}/cart/items`, {
      headers: { ...bearer(user.accessToken), "Content-Type": "application/json" },
      data: { productId: 1, quantity: 1 },
      failOnStatusCode: false,
    });
    expect(r1.status(), `1차 추가 응답: ${r1.status()}`).toBeLessThan(500);

    const r2 = await api.post(`${API_URL}/cart/items`, {
      headers: { ...bearer(user.accessToken), "Content-Type": "application/json" },
      data: { productId: 1, quantity: 1 },
      failOnStatusCode: false,
    });
    expect(r2.status(), `2차 추가 응답: ${r2.status()}`).toBeLessThan(500);

    const cartRes = await api.get(`${API_URL}/cart/me`, {
      headers: bearer(user.accessToken),
      failOnStatusCode: false,
    });
    expect(cartRes.status(), "GET /cart/me가 500이면 NonUniqueResult 결함").not.toBe(500);
    expect(cartRes.status()).toBe(200);

    await api.dispose();
  });

  test("E2E-06-09 신규 user — POST /cart/items 동시 5회 시 5xx 없음 + GET /cart/me 단일 cart 수렴", async () => {
    const api = await playwrightRequest.newContext();
    const user = await registerAndLogin(api, "e2e06-09");

    const concurrentRequests = Array.from({ length: 5 }, () =>
      api.post(`${API_URL}/cart/items`, {
        headers: { ...bearer(user.accessToken), "Content-Type": "application/json" },
        data: { productId: 1, quantity: 1 },
        failOnStatusCode: false,
      }),
    );
    const responses = await Promise.all(concurrentRequests);

    for (const res of responses) {
      expect(res.status(), `동시 추가 응답이 5xx — NonUniqueResult 결함: ${res.status()}`).toBeLessThan(500);
    }

    const cartRes = await api.get(`${API_URL}/cart/me`, {
      headers: bearer(user.accessToken),
      failOnStatusCode: false,
    });
    expect(cartRes.status(), "동시 추가 후 GET /cart/me가 500이면 NonUniqueResult 결함").not.toBe(500);
    expect(cartRes.status()).toBe(200);

    await api.dispose();
  });

  test("E2E-06-R04 GET /cart/me 동시 N회 호출 시 500(NonUniqueResultException) 0건", async () => {
    const api = await playwrightRequest.newContext();
    const user = await registerAndLogin(api, "e2e06-r04");

    await api.post(`${API_URL}/cart/items`, {
      headers: { ...bearer(user.accessToken), "Content-Type": "application/json" },
      data: { productId: 1, quantity: 1 },
      failOnStatusCode: false,
    });

    const concurrentGets = Array.from({ length: 5 }, () =>
      api.get(`${API_URL}/cart/me`, {
        headers: bearer(user.accessToken),
        failOnStatusCode: false,
      }),
    );
    const responses = await Promise.all(concurrentGets);
    for (const res of responses) {
      expect(res.status(), `동시 GET /cart/me가 5xx — NonUniqueResult 결함: ${res.status()}`).not.toBe(500);
      expect(res.status()).toBe(200);
    }

    await api.dispose();
  });

  test("E2E-06-R05 GET /cart/me 호출이 cart row 수를 변경하지 않는다 (조회 전후 활성 cart count 동일)", async () => {
    const api = await playwrightRequest.newContext();
    const user = await registerAndLogin(api, "e2e06-r05");

    await api.post(`${API_URL}/cart/items`, {
      headers: { ...bearer(user.accessToken), "Content-Type": "application/json" },
      data: { productId: 1, quantity: 1 },
      failOnStatusCode: false,
    });

    for (let i = 0; i < 3; i++) {
      const res = await api.get(`${API_URL}/cart/me`, {
        headers: bearer(user.accessToken),
        failOnStatusCode: false,
      });
      expect(res.status(), `${i + 1}회차 GET /cart/me 5xx — 쓰기 사이드이펙트 결함`).not.toBe(500);
      expect(res.status()).toBe(200);
    }

    await api.dispose();
  });

  test("E2E-06-R06 신규 user 의 GET /cart/me 반복 호출 — 동일 cart 반환 (하위호환 확인)", async () => {
    const api = await playwrightRequest.newContext();
    const user = await registerAndLogin(api, "e2e06-r06");

    const res1 = await api.get(`${API_URL}/cart/me`, {
      headers: bearer(user.accessToken),
      failOnStatusCode: false,
    });
    expect(res1.status()).toBe(200);
    const b1 = await res1.json();

    const res2 = await api.get(`${API_URL}/cart/me`, {
      headers: bearer(user.accessToken),
      failOnStatusCode: false,
    });
    expect(res2.status()).toBe(200);
    const b2 = await res2.json();

    if (b1.cartId !== undefined && b2.cartId !== undefined) {
      expect(b2.cartId, "반복 호출에도 동일 cart id 반환").toBe(b1.cartId);
    }

    await api.dispose();
  });

  test("E2E-06-E06 동시 cart 추가 경합에서 응답은 5xx가 아닌 비즈니스 응답(2xx 또는 4xx)이며 DB 중복 cart 없음", async () => {
    const api = await playwrightRequest.newContext();
    const user = await registerAndLogin(api, "e2e06-e06");

    const responses = await Promise.all(
      Array.from({ length: 10 }, () =>
        api.post(`${API_URL}/cart/items`, {
          headers: { ...bearer(user.accessToken), "Content-Type": "application/json" },
          data: { productId: 2, quantity: 1 },
          failOnStatusCode: false,
        }),
      ),
    );

    const has5xx = responses.some((r) => r.status() >= 500);
    expect(has5xx, "동시 경합 중 5xx 발생 — NonUniqueResult 또는 서버 오류").toBe(false);

    const cartRes = await api.get(`${API_URL}/cart/me`, {
      headers: bearer(user.accessToken),
      failOnStatusCode: false,
    });
    expect(cartRes.status()).not.toBe(500);

    await api.dispose();
  });

  test("E2E-06-E07 DELETE /cart 후 POST /cart/items 재추가 시 UNIQUE 제약 위반 500 없음", async () => {
    const api = await playwrightRequest.newContext();
    const user = await registerAndLogin(api, "e2e06-e07");

    await api.post(`${API_URL}/cart/items`, {
      headers: { ...bearer(user.accessToken), "Content-Type": "application/json" },
      data: { productId: 1, quantity: 1 },
      failOnStatusCode: false,
    });

    const del = await api.delete(`${API_URL}/cart`, {
      headers: bearer(user.accessToken),
      failOnStatusCode: false,
    });
    expect([200, 204]).toContain(del.status());

    const reAdd = await api.post(`${API_URL}/cart/items`, {
      headers: { ...bearer(user.accessToken), "Content-Type": "application/json" },
      data: { productId: 1, quantity: 1 },
      failOnStatusCode: false,
    });
    expect(reAdd.status(), "DELETE 후 재추가 시 UNIQUE 제약 위반 500").toBeLessThan(500);

    await api.dispose();
  });
});
